...
const dal = require('../../dals/dals_chats/dal_4')

describe('Testing functionallity of the DAL', () => {
    beforeEach(async () => {
        await dal.create_table_if_not_exist()
        await dal.delete_all()
        await dal.new_message({ 'id': 1, 'customer_id': '1', 'flight_id': '1'}) // id: 1
        await dal.new_message({ 'id': 2, 'customer_id': '2', 'flight_id': '2'}) // id: 2
        await dal.new_message({ 'id': 3, 'customer_id': '3', 'flight_id': '3'}) // Id: 3
        await dal.new_message({ 'id': 4, 'customer_id': '4', 'flight_id': '4'}) // Id: 4
        await dal.new_message({ 'id': 5, 'customer_id': '5', 'flight_id': '5'}) // Id: 5
    })
    ...
    it('update_message', async () => {
        await dal.update_message(3, {  'customer_id': '32', 'flight_id': '32'})
        const expected = '32'
        const message_id_3 = await dal.get_by_id(3)
        const actual = message_id_3.customer_id
        console.log(actual);
    ...
    it('new_message', async () => {
        await dal.new_message({ 'id': 6, 'customer_id': '6', 'flight_id': '6' }) // Id: 6
