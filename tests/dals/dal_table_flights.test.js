...
describe('Testing functionallity of the DAL', () => {
    ...
    //     const actual = await dal.flights_records_tables({'airline_id':9,'origin_country_id':33,'destination_country_id':2,'plane_id':1})
    //     assert.strictEqual(expected, actual.status)
    // })
    // it('flights_records_tables', async () => {
    //     const expected = 'airline_id'
    //     const actual = await dal.flights_records_tables({'airline_id':999,'origin_country_id':2,'destination_country_id':2,'plane_id':1})
    //     assert.strictEqual(expected, actual.status)
    // })
    it('check_flight_existence', async () => {
        const expected = true
        const actual = await dal.check_flight_existence({'airline_id':2,'origin_country_id':3,'destination_country_id':4,'departure_time':'2024-07-03 05:00:00','landing_time':'2024-07-03 10:00:00'})
        assert.strictEqual(expected, actual)
    })
    it('check_flight_existence', async () => {
        const expected = false
        const actual = await dal.check_flight_existence({'airline_id':210,'origin_country_id':2,'destination_country_id':32,'departure_time':'2024-07-03 05:00:00','landing_time':'2024-07-03 10:00:00'})
        assert.strictEqual(expected, actual)
    })
